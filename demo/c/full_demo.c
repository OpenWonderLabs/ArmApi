/*
 * full_demo.c — OneroArm 纯 C ABI：综合运动序列示例
 *
 * 序列：enable
...
int main(void) {
    ...
    onero_sleep_s(1);
    rc |= checked("movej(joint4=0.3)",   onero_movej(h, &joint_target, 1.0, 0));
    onero_sleep_s(1);
    rc |= checked("movep(target)",       onero_movep(h, &pose_target,  1.0, 0));
    onero_sleep_s(1);
    rc |= checked("restore_arm() -> zero", onero_restore_arm(h));
    onero_sleep_s(1);
    rc |= checked("movel(target)",       onero_movel(h, &pose_target,  1.0, 0));
    onero_sleep_s(1);
    rc |= checked("restore_arm() -> zero", onero_restore_arm(h));
